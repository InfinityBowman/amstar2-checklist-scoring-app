...
import { Shape, ShapeStream, ShapeStreamOptions, Row, GetExtensions } from '@electric-sql/client';
import { createSignal, onCleanup, createEffect, on, untrack } from 'solid-js';
...
type ReactiveShapeOptions<SourceData extends Row<unknown>, Selection> =
  | CreateShapeOptions<SourceData, Selection>
  | (() => ShapeStreamOptions<GetExtensions<SourceData>>);

export function createShape<SourceData extends Row<unknown> = Row, Selection = CreateShapeResult<SourceData>>(
  optionsOrFn: ReactiveShapeOptions<SourceData, Selection>,
) {
  const [data, setData] = createSignal<SourceData[]>([]);
  const [isLoading, setIsLoading] = createSignal(true);
  const [isError, setIsError] = createSignal(false);
  const [error, setError] = createSignal<unknown>(null);
  const [lastSyncedAt, setLastSyncedAt] = createSignal<number | undefined>(undefined);
  const [currentShape, setCurrentShape] = createSignal<Shape<SourceData> | null>(null);
  const [currentStream, setCurrentStream] = createSignal<ShapeStream<SourceData> | null>(null);

  let unsubscribe: (() => void) | null = null;
  let latestResult: ShapeDataResult<SourceData> | undefined;

  // Extract selector if options is an object
  const isFunction = typeof optionsOrFn === 'function';
  const selector =
    !isFunction && 'selector' in optionsOrFn && optionsOrFn.selector ?
      optionsOrFn.selector
    : (identity as (arg: CreateShapeResult<SourceData>) => Selection);

  function setupShape(options: ShapeStreamOptions<GetExtensions<SourceData>>) {
  ...
  if (isFunction) {
    // Reactive mode: track signals and recreate shape when they change
    createEffect(
      on(
        () => optionsOrFn(),
        (options) => setupShape(options),
        { defer: false },
      ),
    );
  } else {
    // Static mode: set up once
    const { selector: _selector, ...options } = optionsOrFn;
    setupShape(options as ShapeStreamOptions<GetExtensions<SourceData>>);
  }
  ...
  // Create result object with accessor functions
  const result: CreateShapeResult<SourceData> = {
    data,
    isLoading,
    isError,
