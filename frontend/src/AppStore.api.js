...
import { addUserToProjectByEmail as apiAddUserToProjectByEmail } from './api/projectService.js';
import { searchUsers as apiSearchUsers } from './api/userService.js';
...
export async function addUserToProjectByEmail(projectId, email) {
  try {
    ...
    if (useOnlineStatus()) {
      try {
        // const { addUserToProjectByEmail: apiAddUserToProjectByEmail } = await import('./api/projectService.js');
        const result = await apiAddUserToProjectByEmail(projectId, email);
        return result;
      } catch (apiError) {
